...
describe('Cin7.purchaseOrders', function() {
    before(function() {
        cin7 = new Cin7();
    });
    ...
    describe('#get()', function() {
        context('when using default options', function() {
            it('should create output/purchase-orders.json', function(done) {
                this.timeout(5000);

                cin7.purchaseOrders.get().then((response) => {
                    return response.text();
                }).then((text) => {
                  // make a file system agnostic file name
                  let fileName = 'output' + path.sep + 'purchase-orders.json';
            ...
            it('should create output/purchase-orders.csv', function(done) {
                this.timeout(5000);
                ...
                cin7.purchaseOrders.get().then((response) => {
                    ...
                    // write the csv file
                    let fileName = 'output' + path.sep + 'purchase-orders.csv';
        ...
        context('when using page options', function() {
            it('should get two pages and concat them', function(done) {
                ...
                cin7.purchaseOrders.get(options).then((response) => {
                    return response.json();
                }).then((data) => {
                    ...
                    options.page++;
                    return cin7.purchaseOrders.get(options);
                }).then((response) => {
                    return response.json();
                }).then((data) => {
                    rows = rows.concat(data);
