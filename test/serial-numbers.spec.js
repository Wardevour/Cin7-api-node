...
describe('Cin7.serialNumbers', function() {
    this.timeout(5000);
    ...
    describe('#get()', function() {
        context('when using default options', function() {
            it('should create output/serial-numbers.json', function(done) {
                cin7.serialNumbers.get().then((response) => {
                    return response.text();
                }).then((text) => {
                  // make a file system agnostic file name
                  let fileName = 'output' + path.sep + 'serial-numbers.json';
            ...
            it('should create output/serial-numbers.csv', function(done) {
                cin7.serialNumbers.get().then((response) => {
                    ...
                    // write the csv file
                    let fileName = 'output' + path.sep + 'serial-numbers.csv';
        ...
        context('when using page options', function() {
            it('should get two pages and concat them', function(done) {
                ...
                cin7.serialNumbers.get(options).then((response) => {
                    return response.json();
                }).then((data) => {
                    ...
                    options.page++;
                    return cin7.serialNumbers.get(options);
                }).then((response) => {
                    return response.json();
                }).then((data) => {
                    rows = rows.concat(data);
