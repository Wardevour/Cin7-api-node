...
describe('Cin7.contacts', function() {
    before(function() {
        cin7 = new Cin7();
    });
    ...
    describe('#get()', function() {
        context('when using default options', function() {
            it('should create output/contacts.json', function(done) {
                this.timeout(5000);

                cin7.contacts.get().then((response) => {
                    return response.text();
                }).then((text) => {
                  // make a file system agnostic file name
                  let fileName = 'output' + path.sep + 'contacts.json';
            ...
            it('should create output/contacts.csv', function(done) {
                this.timeout(5000);
                ...
                cin7.contacts.get().then((response) => {
                    ...
                    // write the csv file
                    let fileName = 'output' + path.sep + 'contacts.csv';
        ...
        context('when using page options', function() {
            it('should get two pages and concat them', function(done) {
                ...
                cin7.contacts.get(options).then((response) => {
                    return response.json();
                }).then((data) => {
                    ...
                    options.page++;
                    return cin7.contacts.get(options);
                }).then((response) => {
                    return response.json();
                }).then((data) => {
                    rows = rows.concat(data);
