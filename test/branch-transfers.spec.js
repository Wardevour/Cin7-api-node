...
describe('Cin7.branchTransfers', function() {
    this.timeout(5000);
    ...
    describe('#get()', function() {
        context('when using default options', function() {
            it('should create output/branch-transfers.json', function(done) {
                cin7.branchTransfers.get().then((response) => {
                    return response.text();
                }).then((text) => {
                  // make a file system agnostic file name
                  let fileName = 'output' + path.sep + 'branch-transfers.json';
            ...
            it('should create output/branch-transfers.csv', function(done) {
                cin7.branchTransfers.get().then((response) => {
                    ...
                    // write the csv file
                    let fileName = 'output' + path.sep + 'branch-transfers.csv';
        ...
        context('when using page options', function() {
            it('should get two pages and concat them', function(done) {
                ...
                cin7.branchTransfers.get(options).then((response) => {
                    return response.json();
                }).then((data) => {
                    ...
                    options.page++;
                    return cin7.branchTransfers.get(options);
                }).then((response) => {
                    return response.json();
                }).then((data) => {
                    rows = rows.concat(data);
