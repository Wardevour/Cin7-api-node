...
describe('Cin7.quotes', function() {
    ...
    describe('#get()', function() {
        context('when using default options', function() {
            it('should create output/quotes.json', function(done) {
                cin7.quotes.get().then((response) => {
                    return response.text();
                }).then((text) => {
                  // make a file system agnostic file name
                  let fileName = 'output' + path.sep + 'quotes.json';
            ...
            it('should create output/quotes.csv', function(done) {
                cin7.quotes.get().then((response) => {
                    ...
                    // write the csv file
                    let fileName = 'output' + path.sep + 'quotes.csv';
        ...
        context('when using page options', function() {
            it('should get two pages and concat them', function(done) {
                ...
                cin7.quotes.get(options).then((response) => {
                    return response.json();
                }).then((data) => {
                    ...
                    options.page++;
                    return cin7.quotes.get(options);
                }).then((response) => {
                    return response.json();
                }).then((data) => {
                    rows = rows.concat(data);
